      ...
      if (etablissementsRes.ok) {
        ...
        if (screensRes.ok) {
          ...
          const filteredScreens = allScreens.filter((s: any) =>
            etablissementIds.has(s.etablissementId),
          );
          setScreens(filteredScreens);
        }
      }

      const contentRes = await fetch("/api/backend/content", {
        cache: "no-store",
      });
      if (contentRes.ok) {
  ...
  const openContentModal = async (screen: any) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSelectedContentIds(
          (data || []).map((c: any) => c?._id || c?.id).filter(Boolean),
        );
      }
    } catch (err) {
      console.error(err);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredScreens.map((screen) => (
            <div
              ...
              className="soft-card overflow-hidden group border-border transition-all hover:-translate-y-1 duration-300"
            >
              {/* Card Header (Preview) */}
              <div className="relative h-40 bg-muted/40 overflow-hidden border-b border-border">
                ...
                <div className="absolute top-3 left-3 flex items-center gap-2 bg-background/80 backdrop-blur-md px-2.5 py-1 rounded-full border border-border">
                  <div
                    className={`h-1.5 w-1.5 rounded-full ${screen.status === "Online" || screen.status === "online" ? "bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.8)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]"}`}
                  />
                ...
                <div className="flex justify-between items-start mb-6">
                  <div
                    className={`h-12 w-12 rounded-2xl flex items-center justify-center transition-all ${
                      screen.status === "Online" || screen.status === "online"
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="p-2 bg-primary/10 text-primary hover:bg-primary/20 rounded-xl transition-colors border border-primary/20"
                      title="Modifier"
                    >
                      <Edit2 size={16} />
                    </button>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex flex-col gap-1 mt-1">
                      <p className="text-[10px] text-primary font-bold flex items-center gap-1.5 leading-none tracking-tight">
                        ...
                        {screen.etablissement?.city || "Ville"}
                      </p>
                      <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                        <Building2 size={12} className="text-primary/60" />{" "}
                        {screen.etablissement?.name || "Etablissement locale"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-border grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p
                        className={`text-xs font-mono mt-1 ${!screen.isPaired ? "text-primary font-bold" : "text-muted-foreground opacity-50"}`}
                      >
                        {screen.pairingCode || "---"}
                      </p>
                    </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Mettez à jour les informations de votre périphérique.
                  </p>
                </div>
                <button
            ...
            {submitSuccess ? (
              <div className="p-16 flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-top-4">
                ...
                <p className="text-muted-foreground">
                  Les informations de l'écran ont été actualisées.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  >
                    {myetablissements.map((etablissement) => (
                      <option
            ...
            <div className="p-6 border-b border-border bg-muted/30 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-foreground">
                  Gérer contenus de l'écran
                </h3>
                <p className="text-xs text-muted-foreground mt-1">
                  {screenForContent?.name || "Ecran"}
                </p>
              </div>
              <button
                onClick={() =>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="p-3 rounded-xl bg-muted/30 border border-border">
                  ...
                  <p className="text-sm font-bold mt-1 flex items-center gap-1.5">
                    <span
                      className={`h-2 w-2 rounded-full ${screenForContent?.status === "Online" || screenForContent?.status === "online" ? "bg-emerald-500" : "bg-red-500"}`}
                    />
