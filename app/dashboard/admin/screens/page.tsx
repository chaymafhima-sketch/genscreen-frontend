...
export default function ScreensPage() {
  ...
  const fetchScreens = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Erreur de récupération des écrans: Status ${res.status} - ${text}`);
      }
  ...
  const fetchScreenResolvedContent = async (screenId: string) => {
    try {
      const res = await fetch(`/api/backend/screens/${screenId}/content`, {
        cache: "no-store",
      });
      if (!res.ok)
  ...
  useEffect(() => {
    ...
    fetchEtablissements();
    fetchContents();
    const interval = setInterval(fetchScreens, 30000); // 30s for more real-time feel
  ...
  const openEditModal = (screen: any) => {
    ...
    const establishmentId =
      screen.etablissementId ||
      screen.establishmentId ||
      screen.etablissement?._id ||
      screen.etablissement?.id ||
      screen.establishment?._id ||
      screen.establishment?.id ||
      "";
    setFormData({
      name: screen.name || "",
  ...
  const openAddModal = () => {
    setEditingId(null);
    setFormData({ name: "", establishmentId: "", location: "" });
    setIsModalOpen(true);
  };
  ...
  const openDetailsModal = async (screen: any) => {
    ...
    try {
      const res = await fetch(
        `/api/backend/screens/${screen._id || screen.id}/content`,
        { cache: "no-store" },
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const filteredScreens = screens.filter(
    (s) =>
      ...
      (s.etablissement?.name || s.establishment?.name || "")
        .toLowerCase()
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            >
              ...
              <div className="relative h-44 bg-muted/40 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex items-center gap-2 bg-background/80 backdrop-blur-md px-3 py-1.5 rounded-full border border-border">
                  <div
                    className={`h-2 w-2 rounded-full ${screen.status === "Online" || screen.status === "online" ? "bg-success animate-pulse shadow-[0_0_8px_var(--success)]" : "bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]"}`}
                  />
                ...
                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  ...
                  <button
                    ...
                    className="p-1.5 bg-destructive/80 backdrop-blur-md rounded-lg border border-destructive/30 text-white hover:bg-destructive transition-colors"
                    title="Supprimer"
                  >
                    <Trash2 size={16} />
                  </button>
              ...
              <div className="p-5 space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground flex items-center gap-1.5 mt-1">
                      ...
                      {screen.etablissement?.name ||
                        screen.establishment?.name ||
                        t.common.no_data}
                    </p>
                  </div>
                  <div className="px-2 py-1 bg-primary/10 border border-primary/20 rounded-md">
                    <span className="text-[9px] font-black text-primary uppercase">
                      ID:{" "}
                      {screen.id?.slice(-4) || screen._id?.slice(-4) || "????"}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-4 border-t border-border/50">
                  <div>
                    <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-tighter">
                      {t.screens.card.pairing_code}
                    </p>
                    <p
                      className={`text-xs font-mono mt-0.5 ${!screen.isPaired ? "text-primary font-bold" : "text-muted-foreground opacity-50"}`}
                    >
                      {screen.pairingCode || "---"}
                      {!screen.isPaired && (
                        <span className="ml-2 text-[8px] bg-primary/20 px-1 rounded">
                          {t.screens.card.to_pair}
                        </span>
                      )}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">
                      {t.screens.card.last_activity}
                    </p>
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {screen.lastSeen || t.screens.card.unknown}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2 border-t border-border/10">
                  ...
                  <div className="text-right">
                    <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-tighter">
                      {t.screens.card.serial_number}
                    </p>
                    <p className="text-xs text-foreground font-mono mt-0.5 truncate">
                      {screen.serialNumber || t.screens.card.not_linked}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-2xl font-bold text-foreground tracking-tight">
                    {editingId ? "Modifier l'écran" : "Ajouter un écran"}
                  </h2>
                  <p className="text-sm text-muted-foreground mt-1">
                    {editingId
                      ? "Mettez à jour les informations de l'écran."
                      : "Enregistrez un nouveau périphérique sur votre réseau."}
                  </p>
                </div>
                <button
            ...
            {submitSuccess ? (
              <div className="p-16 flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-top-4">
                ...
                <p className="text-muted-foreground">
                  L'écran a été configuré et peut maintenant recevoir des flux.
                </p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    {etablissements.map((etablissement) => (
                      <option
                        ...
                        value={etablissement._id || etablissement.id}
                      >
                        {etablissement.name}
                      </option>
                    ))}
            ...
            <div className="p-6 border-b border-border bg-muted/30 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {screenForContent?.name || "Écran"} - sélectionnez des
                  contenus puis choisissez l'action.
                </p>
              </div>
              <button
                onClick={() =>
                  !isAssigningContent && setIsContentModalOpen(false)
                }
                className="text-muted-foreground hover:text-foreground transition-colors"
              >
                <X size={20} />
      ...
      {isDetailsModalOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-background/60 backdrop-blur-md"
            onClick={() => !isAssigningContent && setIsDetailsModalOpen(false)}
          />
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`h-10 w-10 rounded-xl flex items-center justify-center ${
                    screenForContent?.status === "Online" ||
                    screenForContent?.status === "online"
                ...
                <div>
                  <h3 className="text-lg font-bold text-foreground">
                    {screenForContent?.name}
                  </h3>
                  <p className="text-xs text-muted-foreground">
                    Configuration de la diffusion (ADMIN)
                  </p>
                </div>
              </div>
              <button
                onClick={() =>
                  !isAssigningContent && setIsDetailsModalOpen(false)
                }
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="p-3 rounded-xl bg-muted/30 border border-border">
                  ...
                  <p className="text-sm font-bold mt-1 flex items-center gap-1.5">
                    <span
                      className={`h-2 w-2 rounded-full ${screenForContent?.status === "Online" || screenForContent?.status === "online" ? "bg-success" : "bg-red-500"}`}
                    />
                ...
                <div className="space-y-3">
                  {currentPlaylist.length === 0 ? (
                    <div className="py-12 flex flex-col items-center justify-center border-2 border-dashed border-border rounded-2xl text-muted-foreground">
                      ...
                      <p className="text-sm">
                        Aucun contenu assigné à cet écran.
                      </p>
                    </div>
                  ) : (
                    currentPlaylist.map((item, idx) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            onClick={() => {
                              const nextP = currentPlaylist.filter(
                                (_, i) => i !== idx,
                              );
                              setCurrentPlaylist(nextP);
                            }}
            ...
            <div className="flex gap-4 w-full">
              ...
              >
               ...
               <button
                ...
                disabled={isDeleting}
                className="flex-1 py-3.5 bg-muted/40 text-foreground hover:bg-muted border border-border rounded-2xl font-bold transition-all flex justify-center items-center gap-2"
              >
